// elm-watch hot {"version":"1.1.2","targetName":"My target name","webSocketPort":39619}
"use strict";
(() => {
...
var $author$project$Main$AcademicLiteracy = {$: 'AcademicLiteracy'};
var $author$project$Main$CourseStructure = {$: 'CourseStructure'};
var $author$project$Main$MentalModel = {$: 'MentalModel'};
var $author$project$Main$PoorAssessmentBehaviour = {$: 'PoorAssessmentBehaviour'};
var $author$project$Main$PoorClassBehaviour = {$: 'PoorClassBehaviour'};
var $author$project$Main$PoorOutOfClassBehaviour = {$: 'PoorOutOfClassBehaviour'};
var $author$project$Main$PracticeRelated = {$: 'PracticeRelated'};
var $author$project$Main$Problem = F6(
...
var $author$project$Main$SubjectSpecificComprehension = {$: 'SubjectSpecificComprehension'};
var $author$project$Main$defaultProblems = _List_fromArray(
	[
		A6(
		$author$project$Main$Problem,
		...
		'',
		$author$project$Main$PracticeRelated,
		_List_Nil,
		_List_fromArray(
			[26, 27])),
		A6(
		...
		'',
		$author$project$Main$CourseStructure,
		_List_Nil,
		_List_fromArray(
			[1, 9, 2])),
		A6(
		...
		'',
		$author$project$Main$AcademicLiteracy,
		_List_Nil,
		_List_fromArray(
			[7, 8, 15])),
		A6($author$project$Main$Problem, 'Don\'t understand how to construct a STATEMENT: a full, coherent sentence that expresses a full idea', '', '', $author$project$Main$AcademicLiteracy, _List_Nil, _List_Nil),
		A6($author$project$Main$Problem, 'Students just treat formulae as opaque things, with no interpretation.', 'If the formula doesn\'t give the result immediately, or there is no obvious formula to apply, then the student is stumped.', 'This appears a lot in Computer Science. If a technique shown in class needs\n    even slight modification to work, e.g. nesting loops or breaking out of a loop\n    in a different place, then some students just draw a blank.\n    ', $author$project$Main$SubjectSpecificComprehension, _List_Nil, _List_Nil),
		A6($author$project$Main$Problem, 'Students don\'t differentiate properly between big overarching principles and special ways / shortcuts.', 'Students either apply first-principles inappropriately, OR apply shortcuts inappropriately.', '', $author$project$Main$SubjectSpecificComprehension, _List_Nil, _List_Nil),
		A6(
		$author$project$Main$Problem,
		'Students can\'t construct a logical argument',
		'This is an argument with premises and conclusion(s), based on accepted axioms',
		'',
		$author$project$Main$SubjectSpecificComprehension,
		_List_Nil,
		_List_fromArray(
			[8])),
		A6(
		...
		_List_fromArray(
			[12, 3])),
		A6($author$project$Main$Problem, 'Students think that the point is maximising marks.', 'Don\'t see the point of assignments, or resources put up for them, or feedback, or university.  Don\'t see the point of an accumulation of knowledge & skill.', '', $author$project$Main$MentalModel, _List_Nil, _List_Nil),
		A6($author$project$Main$Problem, 'Students don\'t understand overloaded / contextual mathematical or notational dialects.', 'This includes understanding that they exist at all.  Different maths symbols can mean different things in different contexts; it\'s like a language with dialects AND overloaded symbols.  Students don\'t understand the differences and don\'t understand different meanings in their context.', '', $author$project$Main$MentalModel, _List_Nil, _List_Nil),
		A6(
		$author$project$Main$Problem,
		...
		$author$project$Main$MentalModel,
		_List_Nil,
		_List_fromArray(
			[3, 11])),
		A6(
		...
		_List_fromArray(
			[21, 22, 24, 0, 25])),
		A6($author$project$Main$Problem, 'Students rely on external sources (e.g. YouTube or notes) rather than textbooks and \"offical\" sources', 'Non-official sources can lead them astray (sometimes obviously, sometimes subtly) and it can be difficult to pick this up.', '', $author$project$Main$PoorOutOfClassBehaviour, _List_Nil, _List_Nil),
		A6(
		$author$project$Main$Problem,
		'Students ignore / don\'t engage with feedback on assignments/tests',
		'',
		'',
		$author$project$Main$PoorOutOfClassBehaviour,
		_List_Nil,
		_List_fromArray(
			[17, 29, 28])),
		A6($author$project$Main$Problem, 'Students stick to basics (high-school or earliest-taught) and resist the use of new things when they come in', '', '', $author$project$Main$PoorAssessmentBehaviour, _List_Nil, _List_Nil),
		A6(
		$author$project$Main$Problem,
		'Students don\'t work at the speed required to pass an assessment',
		'',
		'',
		$author$project$Main$PoorAssessmentBehaviour,
		_List_Nil,
		_List_fromArray(
			[10])),
		A6(
...
var $author$project$Main$initialModel = {expandedIndex: $elm$core$Maybe$Nothing, problems: $author$project$Main$defaultProblems};
...
var $author$project$Main$allCategories = _List_fromArray(
	[$author$project$Main$PracticeRelated, $author$project$Main$CourseStructure, $author$project$Main$AcademicLiteracy, $author$project$Main$SubjectSpecificComprehension, $author$project$Main$MentalModel, $author$project$Main$PoorClassBehaviour, $author$project$Main$PoorOutOfClassBehaviour, $author$project$Main$PoorAssessmentBehaviour]);
...
var $author$project$Main$categoryName = function (category) {
	switch (category.$) {
...
var $author$project$Main$categoryToColor = function (category) {
	switch (category.$) {
		case 'PracticeRelated':
			return '#B62045';
		case 'CourseStructure':
...
var $author$project$Main$ExpandProblem = function (a) {
	return {$: 'ExpandProblem', a: a};
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					'backgroundColor',
					$author$project$Main$categoryToColor(problem.category)),
					$elm_community$string_extra$String$Extra$isBlank(problem.detail) ? $elm$html$Html$Attributes$class('no-detail') : $elm$html$Html$Attributes$title(problem.detail),
					$elm$html$Html$Events$onClick(
					$author$project$Main$ExpandProblem(index))
				]),
			_List_fromArray(
				[
				...
				A2(
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'color',
							$author$project$Main$categoryToColor(category))
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(
							$author$project$Main$categoryName(category))
						])),
				A2(
					$elm$core$List$indexedMap,
					$author$project$Main$viewProblem,
					A2(
